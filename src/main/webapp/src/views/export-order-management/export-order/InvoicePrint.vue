<template>
  <div class="invoice-box"  ref="invoiceContent">
    ...
    <!-- Tiêu đề hóa đơn -->
    <div class="invoice-title">HÓA ĐƠN XUẤT HÀNG {{ order.type === 'INTERNAL' ? '(NỘI BỘ)' : '' }}</div>
    ...
    <table>
      <tbody>
        ...
        <tr>
          <td colspan="2"><strong>Khách hàng:</strong> {{ customer()?.name }}</td>
        </tr>
        <tr v-if="order.type === 'EXPORT'">
          <td colspan="2"><strong>SĐT:</strong> {{ customer()?.phoneNumber }}</td>
        </tr>
        <tr>
          <td colspan="2"><strong>Địa chỉ:</strong> {{ customer()?.address }}</td>
        </tr>
      </tbody>
    </table>
...
const props = defineProps<{
  order: ExportOrder.Data,
  referenceData: ReferenceData.ExportOrder
}>()
...
function customer() {
  if (props.order.type === 'EXPORT') {
    const customer = props.referenceData.customers.find(c => c.id === props.order.customerId)
    return {
      name: customer?.name,
